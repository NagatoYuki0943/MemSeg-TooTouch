...
class Decoder(nn.Module):
    ...
    def forward(self, encoder_output, concat_features):
        # concat_features = [level0, level1, level2, level3] [[B, 64, 128, 128], [B, 64, 64, 64], [B, 128, 32, 32], [B, 256, 16, 16]]
        f0, f1, f2, f3 = concat_features

        x_up3 = self.upconv3(encoder_output)        # [B, 512, 8, 8] -> [B, 256, 16, 16]
        x_up3 = torch.cat([x_up3, f3], dim=1)       # cat([B, 256, 16, 16], [B, 256, 16, 16]) = [B, 512, 16, 16]

        x_up2 = self.upconv2(x_up3)                 # [B, 512, 16, 16] -> [B, 128, 32, 32]
        x_up2 = torch.cat([x_up2, f2], dim=1)       # cat([B, 128, 32, 32], [B, 128, 32, 32]) = [B, 256, 32, 32]

        x_up1 = self.upconv1(x_up2)                 # [B, 256, 32, 32] -> [B, 64, 64, 64]
        x_up1 = torch.cat([x_up1, f1], dim=1)       # cat([B, 64, 64, 64], [B, 64, 64, 64]) = [B, 128, 64, 64]

        x_up0 = self.upconv0(x_up1)                 # [B, 128, 64, 64]  -> [B, 48, 128, 128]
        f0 = self.conv(f0)                          # [B, 64, 128, 128] -> [B, 48, 128, 128]
        x_up2mask = torch.cat([x_up0, f0], dim=1)   # cat([B, 48, 128, 128], [B, 48, 128, 128]) = [B, 96, 128, 128]
